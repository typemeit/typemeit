...
/// (docs/meetings.md 8.6). Written to `MeetingProbes.directory` as it goes,
/// so a quit keeps what was read; it holds whatever the window shows and
/// stays on this Mac.
@MainActor
@Observable
final class WindowCapture {
    ...
    private final class Run: @unchecked Sendable {
        ...
        init(pid: pid_t, target: Roster.Target) {
            ...
            header = "\(target.rawValue) window, from \(ISO8601DateFormatter().string(from: Date())), every \(Fixed.meetingSpeakingPollMs) ms"
        }
        ...
        private func tick(done: @Sendable (URL?) -> Void) {
            ...
            if let previous {
                ...
                if !gone.isEmpty || !came.isEmpty { append(["t=\(ms)"] + gone.map { "- " + $0 } + came.map { "+ " + $0 }) }
            } else {
                append([header, "snapshot at \(ms) ms"] + current)
            }
            previous = now
            guard ms >= WindowCapture.seconds * 1000 else { return }
            timer?.cancel()
            timer = nil
            append(["snapshot at \(ms) ms"] + current)
            try? handle?.close()
            done(handle == nil ? nil : file)
        }
