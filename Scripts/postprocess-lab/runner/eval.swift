...
@main struct Eval {
    ...
    static func pct(_ xs: [Double], _ q: Double) -> Double {
        let s = xs.sorted(); return s.isEmpty ? 0 : s[min(s.count - 1, Int(Double(s.count) * q))]
    }

    static func main() async throws {
        let env = ProcessInfo.processInfo.environment
        let casesURL = URL(fileURLWithPath: env["EVAL_CASES"] ?? "cases.json")
        let outDir = URL(fileURLWithPath: env["EVAL_OUT"] ?? ".")
        try FileManager.default.createDirectory(at: outDir, withIntermediateDirectories: true)
        // cold: a new session per run, as the stock eval does. prewarm: prepare()
        // first and wait, as the app does while the user speaks.
        let mode = env["EVAL_MODE"] ?? "cold"
        let prewarmWait = Duration.milliseconds(Int(env["EVAL_PREWARM_MS"] ?? "1500")!)
        let skipScreenBlind = env["EVAL_SKIP_BLIND"] == "1"
        let cases = try JSONDecoder().decode([Case].self, from: Data(contentsOf: casesURL))
        guard case .available = PostProcessor.availability else { print("Apple Intelligence is not available on this Mac"); exit(2) }
        // Warm-up: loads the model so the first timed run does not pay for it.
        _ = await PostProcessor.shared.clean(CustomWordMatcher.apply([CustomWordMatcher.Word(text: "hello", confidence: 1)], terms: [String]()), customWords: [])
        ...
        var timings: [Double] = [], appliedCount = 0
        for c in cases {
            let terms = await MainActor.run { ScreenContext.terms(from: c.screen ?? [], excluding: c.customWords) }
            for r in c.runs {
                total += 1
                if mode == "prewarm" {
                    await PostProcessor.shared.prepare(screenTerms: terms, styles: r.styles)
                    try await Task.sleep(for: prewarmWait)
                }
                let t0 = ContinuousClock.now
                let cleaned = await PostProcessor.shared.clean(c.matched, customWords: c.customWords, screenTerms: terms, styles: r.styles)
                let dt = ms(ContinuousClock.now - t0)
                timings.append(dt)
                if cleaned.applied { appliedCount += 1 }
                ...
                print("\(c.wish != nil ? (ok ? "WISH  granted" : "WISH  not yet") : (ok ? "PASS" : "FAIL"))  \(String(format: "%5.0f", dt)) ms\(cleaned.applied ? "" : " (not applied)")  \(r.input)\(tag)")
                if !ok { print("      expected: \(r.accepted.joined(separator: "\n             or: "))\n      got:      \(out)") }
                var blind: String?, blindOk: Bool?
                if c.screen != nil, !skipScreenBlind {
                    screened += 1
                ...
                results.append(Result(input: r.input, output: out, styles: r.styles.map(\.rawValue).sorted(), pass: ok, wish: c.wish, matched: matched, expected: r.accepted, whys: r.whys, screen: c.screen, terms: terms, withoutScreen: blind, withoutScreenPass: blindOk, ms: dt, applied: cleaned.applied))
            }
        }
        let wall = ms(ContinuousClock.now - wallStart) / 1000
        ...
        print(String(format: "latency ms (%@): p50 %.0f  p90 %.0f  p95 %.0f  max %.0f  mean %.0f  over %d runs; model output applied %d; wall %.0f s", mode, pct(timings, 0.5), pct(timings, 0.9), pct(timings, 0.95), timings.max() ?? 0, mean, timings.count, appliedCount, wall))
        let enc = JSONEncoder(); enc.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]
        try enc.encode(results).write(to: outDir.appendingPathComponent("result.json"))
        try html(results, passed: total - failed, total: total, granted: granted, wished: wished).write(to: outDir.appendingPathComponent("evals.html"), atomically: true, encoding: .utf8)
        exit(failed == 0 ? 0 : 1)
    }
}
